...
**All scripts in this guide should be run within the context of a task directory.** Before running any of the CLI integration scripts, ensure you have initialized a task:

```powershell
# Initialize a new task first
. .orchestrator/init-orchestrator.ps1 -Slug "my-task"

# Then the scripts below will use .orchestrator/latest as the working directory
```

The scripts use `.orchestrator/latest` as the default task directory. Each task has its own isolated:
- `agent_tasks/` - Agent task files
- `results/` - Agent execution results
- `master_plan.md` - Task planning document
...
$task = Get-Content "$taskDir/agent_tasks/agent-01.md" -Raw
claude -p $task

# Save result to file
claude -p $task | Out-File "$taskDir/results/agent-01-result.md" -Encoding UTF8
```
...
```powershell
# Launch multiple Agents in parallel (with task isolation)
$taskDir = ".orchestrator/latest"
$taskFiles = Get-ChildItem "$taskDir/agent_tasks/*.md"
...
$jobs = foreach ($file in $taskFiles) {
    ...
    } -ArgumentList $file.FullName, "$taskDir/results/$agentId-result.md"
}
